...
def extract_year_from_title(title: str) -> Optional[int]:
    match = re.search('\\b(20[0-2]\\d|199\\d)\\b', title)
    if match:
...
def fetch_s_tier_tournaments(start_year: int, end_year: int, delay: float, limit: Optional[int], force_refresh: bool):
    ...
    for page in all_members:
        ...
        if not cached_data:
            ...
            cached_data = {'title': title, 'pagename': pagename, 'info': info, 'team_cards': team_cards, 'prize_pool': prize_pool, 'wikitext': wikitext}
        ...
        if not start_year <= year <= end_year:
            skipped_count += 1
            continue
        tourney_record = {'id': pagename.lower(), 'name': info.get('name', title), 'start_date': start_date, 'end_date': info.get('end_date'), 'year': year, 'tier': info.get('tier', 'S-Tier'), 'format': info.get('format'), 'liquipedia_url': f'https://liquipedia.net/counterstrike/{pagename}', 'raw_json_path': str(RawStorageManager._clean_filename(pagename))}
        ...
        for tc in team_cards_live:
            ...
            db.add_tournament_result({'tournament_id': pagename.lower(), 'team_name': team_name, 'placement': placement, 'placement_numeric': None, 'prize_money': None})
            for p in tc.get('players', []):
                p_id = p['player_id']
                db.upsert_player({'id': p_id, 'handle': p['handle']})
                db.add_appearance({'player_id': p_id, 'tournament_id': pagename.lower(), 'team_name': team_name, 'role': p.get('role', 'Rifler'), 'placement': placement, 'year': year})
        processed_count += 1
...
def seed_sample_data():
    db = DatabaseManager()
    sample_tournaments = [{'id': 'eleague_major_atlanta_2017', 'name': 'ELEAGUE Major: Atlanta 2017', 'start_date': '2017-01-22', 'end_date': '2017-01-29', 'year': 2017, 'tier': 'Major', 'format': 'Swiss + Single Elimination', 'liquipedia_url': 'https://liquipedia.net/counterstrike/ELEAGUE/2017/Major'}, {'id': 'pgl_major_krakow_2017', 'name': 'PGL Major Kraków 2017', 'start_date': '2017-07-16', 'end_date': '2017-07-23', 'year': 2017, 'tier': 'Major', 'format': 'Swiss + Single Elimination', 'liquipedia_url': 'https://liquipedia.net/counterstrike/PGL/2017/Krakow'}, {'id': 'esl_one_cologne_2017', 'name': 'ESL One Cologne 2017', 'start_date': '2017-07-04', 'end_date': '2017-07-09', 'year': 2017, 'tier': 'S-Tier', 'format': 'Swiss + Single Elimination', 'liquipedia_url': 'https://liquipedia.net/counterstrike/ESL/One/2017/Cologne'}, {'id': 'iem_katowice_2018', 'name': 'IEM Katowice 2018', 'start_date': '2018-02-27', 'end_date': '2018-03-04', 'year': 2018, 'tier': 'S-Tier', 'format': 'GSL + Single Elimination', 'liquipedia_url': 'https://liquipedia.net/counterstrike/IEM/Katowice/2018'}, {'id': 'faceit_major_london_2018', 'name': 'FACEIT Major: London 2018', 'start_date': '2018-09-05', 'end_date': '2018-09-23', 'year': 2018, 'tier': 'Major', 'format': 'Swiss + Single Elimination', 'liquipedia_url': 'https://liquipedia.net/counterstrike/FACEIT/2018/Major'}]
    for t in sample_tournaments:
        db.upsert_tournament(t)
    db.upsert_player({'id': 'fallen', 'handle': 'FalleN', 'real_name': 'Gabriel Toledo', 'birth_date': '1991-05-30', 'nationality': 'Brazil', 'photo_url': 'https://liquipedia.net/commons/Special:FilePath/FalleN_at_IEM_Katowice_2020.jpg'})
    db.upsert_player({'id': 'coldzera', 'handle': 'coldzera', 'real_name': 'Marcelo David', 'birth_date': '1994-10-31', 'nationality': 'Brazil', 'photo_url': 'https://liquipedia.net/commons/Special:FilePath/Coldzera_2019.jpg'})
    sample_appearances = [{'player_id': 'fallen', 'tournament_id': 'eleague_major_atlanta_2017', 'team_name': 'SK Gaming', 'role': 'IGL / AWPer', 'placement': '3-4', 'year': 2017}, {'player_id': 'fallen', 'tournament_id': 'esl_one_cologne_2017', 'team_name': 'SK Gaming', 'role': 'IGL / AWPer', 'placement': '1', 'year': 2017}, {'player_id': 'fallen', 'tournament_id': 'pgl_major_krakow_2017', 'team_name': 'SK Gaming', 'role': 'IGL / AWPer', 'placement': '5-8', 'year': 2017}, {'player_id': 'coldzera', 'tournament_id': 'iem_katowice_2018', 'team_name': 'SK Gaming', 'role': 'Rifler', 'placement': '5-6', 'year': 2018}, {'player_id': 'coldzera', 'tournament_id': 'faceit_major_london_2018', 'team_name': 'MIBR', 'role': 'Rifler', 'placement': '3-4', 'year': 2018}]
