...
class LiquipediaAPIClient:
    ...
    def fetch_category_members(self, category_name: str, limit: int = 500) -> List[Dict[str, Any]]:
        ...
        Uses action=query (lightweight, 2s rate limit).
        """
        ...
        while True:
            params = {
                ...
                "cmtitle": cat_title,
                "cmlimit": str(min(limit - len(all_members), 500)),
                "cmtype": "page"
            }
            if cm_continue:
    ...
    def fetch_page_wikitext(self, page_title: str) -> Optional[str]:
        ...
        This is a LIGHTWEIGHT request (2s rate limit), unlike action=parse (30s).
        """
        params = {
            "action": "query",
            "titles": page_title,
