...
def build_liquipedia_image_url(image_name: str) -> Optional[str]:
    ...
    Example: 'AcAp at OMEN.JPG' -> 'https://liquipedia.net/commons/images/9/9d/AcAp_at_OMEN.JPG'
    Returns valid direct image URL that opens cleanly with HTTP 200 in all browsers.
    """
    if not image_name:
        return None
...
def get_team_placement(team_name: str, prize_map: Dict[str, str]) -> str:
    ...
    Handles exact names, acronyms (lg, nip, navi), and substrings.
    """
    if not prize_map:
        return "5-8"
...
class WikitextParser:
    ...
    def _split_template_params(block: str) -> Dict[str, str]:
        """
        Splits a wikitext template block into key=value pairs.
        Handles multiple params on the same line separated by | using safe_split_params.
        """
        params = {}
    ...
    def parse_prize_pool(wikitext: str) -> Dict[str, str]:
        """
        Parses prize pool slots into a placement map.
        Handles named (team=...) and unnamed (|lg, |navi) positional parameters.
        """
        ...
        for slot in slots:
            ...
            for token in tokens:
                if "=" in token:
                    k, v = token.split("=", 1)
                    k = k.strip().lower()
                    v_clean = re.sub(r"\[\[(?:[^|]*\|)?([^\]]+)\]\]", r"\1", v.strip()).strip()
