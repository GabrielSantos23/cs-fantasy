    ...
    def enrich_player(self, player_id: str, force_refresh: bool=False) -> Dict[str, Any]:
        ...
        if not raw_data:
            ...
            if wikitext:
                ...
                raw_data = {'source': 'wikitext', 'handle': parsed.get('handle', player_id), 'real_name': parsed.get('real_name'), 'birth_date': parsed.get('birth_date'), 'nationality': parsed.get('nationality'), 'photo_url': parsed.get('photo_url'), 'role': parsed.get('role')}
        ...
        if not raw_data:
            ...
            basic_player = {'id': player_id_clean, 'handle': player_id, 'real_name': None, 'birth_date': None, 'nationality': None, 'photo_url': None, 'liquipedia_url': f'https://liquipedia.net/counterstrike/{player_id}'}
        ...
        player_record = {'id': player_id_clean, 'handle': raw_data.get('handle') or player_id, 'real_name': raw_data.get('real_name'), 'birth_date': raw_data.get('birth_date'), 'nationality': raw_data.get('nationality'), 'photo_url': photo_url, 'liquipedia_url': f"https://liquipedia.net/counterstrike/{raw_data.get('handle', player_id)}"}
