    ...
    def score_era(cls, era_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for app in appearances:
            ...
            if team_name not in team_breakdown:
                team_breakdown[team_name] = {'tournaments': 0, 'score': 0.0, 'placements': []}
            ...
            tournaments_breakdown.append({'tournament_id': app.get('tournament_id'), 'tournament_name': tournament_name, 'tier': tier, 'team': team_name, 'role': app.get('role'), 'placement': placement, 'score': score})
        count = len(appearances)
        avg_score = round(total_score / count, 2) if count > 0 else 0.0
        total_score = round(total_score, 2)
        return {'player_id': era_data['player_id'], 'year': era_data['year'], 'total_score': total_score, 'average_score': avg_score, 'tournaments_count': count, 'teams': era_data.get('teams', []), 'roles': era_data.get('roles', []), 'breakdown': {'team_summary': team_breakdown, 'tournaments': tournaments_breakdown}}
