...
def test_upsert_player_and_era(tmp_path):
    db_file = tmp_path / 'test_cs.db'
    db = DatabaseManager(db_path=db_file)
    player_id = db.upsert_player({'id': 's1mple', 'handle': 's1mple', 'real_name': 'Oleksandr Kostyliev', 'nationality': 'Ukraine'})
    assert player_id == 's1mple'
    db.upsert_player_era({'player_id': 's1mple', 'year': 2021, 'total_score': 500.0, 'average_score': 100.0, 'tournaments_count': 5, 'teams': ['Natus Vincere'], 'roles': ['AWPer'], 'breakdown': {'team_summary': {'Natus Vincere': {'score': 500.0}}}})
