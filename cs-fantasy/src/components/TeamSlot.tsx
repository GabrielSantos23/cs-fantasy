...
export const TeamSlot: React.FC<TeamSlotProps> = ({
  ...
  if (era) {
    ...
    return (
      <div 
        onClick={onRemove}
        className="group relative h-[260px] w-full rounded-xl overflow-hidden border border-white/10 bg-bg-card transition-all duration-300 hover:border-red-500/40 hover:bg-red-950/10 cursor-pointer"
      ...
      >
        ...
        {/* Player photo — centered, cropped from waist up */}
        {era.photo_url && !imgError ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center z-[2]">
            <div className={`size-16 rounded-full ${roleColor}/30 border border-white/20 flex items-center justify-center shadow-lg backdrop-blur-sm group-hover:scale-110 transition-transform duration-300`}>
        ...
        {/* Yellow/gold gradient overlay from top to bottom */}
        <div
          className="absolute inset-0 z-[3] pointer-events-none"
          style={{
            background: "linear-gradient(to bottom, rgba(234, 179, 8, 0.15) 0%, rgba(234, 179, 8, 0.05) 25%, transparent 50%, rgba(0, 0, 0, 0.6) 75%, rgba(0, 0, 0, 0.92) 100%)",
          }}
        />
        ...
        <div className="absolute top-2.5 right-2.5 z-10 flex flex-col items-end gap-1">
          <div className="bg-black/60 backdrop-blur-sm border border-hi/30 rounded-lg px-2 py-1 text-right">
            ...
            <span className="text-hi/60 text-[9px] font-bold ml-0.5">
              PTS
            </span>
          </div>
        </div>
        ...
        <div className="absolute top-2.5 left-1/2 -translate-x-1/2 z-10">
          <Badge className={`flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider text-white px-1.5 py-0.5 rounded-md border-0 ${roleColor}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 p-3 space-y-0.5">
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs text-white/50 truncate">{mainTeam || era.real_name}</p>
            <span className="text-[10px] text-white/40 font-mono">{era.year}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div
      onClick={onClickSlot}
      className={`h-[260px] w-full rounded-xl border-2 border-dashed border-white/10 bg-white/[0.02] flex flex-col items-center justify-center gap-2 transition-all duration-300 ${
        onClickSlot
          ? "cursor-pointer hover:border-hi/40 hover:bg-hi/[0.03]"
          : ""
      }`}
    >
