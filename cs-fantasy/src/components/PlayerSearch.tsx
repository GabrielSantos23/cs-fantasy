...
function getRoleIcon(role: string) {
  switch (role) {
    case "IGL": return <Crosshair className="size-3" />;
    case "AWPer":
    case "AWP": return <Target className="size-3" />;
    case "Rifler": return <Swords className="size-3" />;
    case "Entry": return <Shield className="size-3" />;
    case "Support": return <Users className="size-3" />;
    case "Lurker": return <Eye className="size-3" />;
    default: return null;
  }
}
...
export const PlayerSearch: React.FC<PlayerSearchProps> = ({
  ...
  // Filtered eras (strictly within 2013–2026 range)
  const filteredEras = useMemo(() => {
    let results = dataset.erasList.filter((era) => {
      // Must be between 2013 and 2026
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-9 bg-white/[0.03] border-white/8 text-white placeholder:text-white/25 focus-visible:ring-hi/40 h-9"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="w-[180px] bg-white/[0.03] border-white/8 text-white h-9 text-xs font-medium focus:ring-hi/40">
                      ...
                      <Button
                        ...
                        className={`text-[10px] font-bold uppercase tracking-wider cursor-pointer ${
                          isSelected
                            ...
                            : "bg-white/[0.04] border-white/10 text-white/60 hover:bg-hi/10 hover:text-hi hover:border-hi/30"
                        }`}
                        title={isSelected ? "Clique para remover do roster" : "Clique para selecionar"}
                      >
                        {isSelected ? (
                          <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 pr-3">
            {visibleEras.map((era) => {
              ...
              return (
                <div
                  key={era.id}
                  className={`group relative h-[220px] w-full rounded-xl overflow-hidden border bg-[#111111]/80 backdrop-blur-md transition-all duration-300 cursor-pointer ${
                    isSelected
                      ? "border-white/40 ring-1 ring-white/20 bg-white/[0.08]"
                      : "border-white/[0.08] hover:border-white/20 hover:bg-[#161616] hover:-translate-y-1 hover:shadow-xl hover:shadow-black/50"
                  }`}
                  onClick={() => onSelectEra(era, asCoachMode)}
                  title={isSelected ? "Clique para remover do roster" : "Clique para selecionar"}
                >
                  ...
                  {(() => {
                    const logoInfo = (logoMapData as Record<string, { local_path: string | null; remote_url: string | null }>)[mainTeam];
                    const logoUrl = logoInfo?.local_path || logoInfo?.remote_url || null;
                    return logoUrl ? (
                  ...
                  {/* Player photo or Fallback Avatar */}
                  {era.photo_url && !hasImgError ? (
                    <img
                      src={
                        era.photo_url.startsWith("http")
                  ...
                  <div className="absolute top-2 right-2 z-10">
                    <div className="bg-white/5 border border-white/10 rounded-lg px-2 py-0.5 text-right backdrop-blur-md">
                      <span className="text-white font-bold text-sm leading-none" style={{ fontFamily: "var(--font-outfit)" }}>
                        {era.total_score.toFixed(0)}
                      </span>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 z-10 p-2.5 space-y-0.5">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-[11px] text-white/35 truncate">{mainTeam || era.real_name}</p>
                      <span className="text-[10px] text-white/40 font-mono font-medium">{era.year}</span>
                    </div>
                  </div>
                </div>
              );
